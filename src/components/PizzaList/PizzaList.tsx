...
import { useGetAllPizzasQuery, useGetAllPizzasSortingQuery } from "@/services/pizza.api";
import { useTypedSelector } from "@/hooks/useTypedSelector";

const PizzaList: FC = () => {
    const { filter, sorting } = useTypedSelector(state => state.filterReducer)
    const { isLoading, isError, data: pizzas } = useGetAllPizzasSortingQuery({sorting})
